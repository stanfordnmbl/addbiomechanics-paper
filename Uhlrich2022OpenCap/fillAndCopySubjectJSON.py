...
def fillAndCopySubjectJSON():
    ...
    # under 'Formatted'.
    for subject, mass, height, sex in zip(subjects, masses, heights, sexes):
        if not os.path.isdir(os.path.join('Formatted', subject)):
            os.mkdir(os.path.join('Formatted', subject))

        # Load template.
        f = open(os.path.join('templates', '_subject.json'))
        data = json.load(f)
        ...
        data['segmentTrials'] = True
        # data['tuneResidualLoss'] = 0.1
        # data['dynamicsMarkerWeight'] = 250.0
        # data['dynamicsRegularizePoses'] = 0.1
